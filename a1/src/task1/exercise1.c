...
int futhark_entry_process(struct futhark_context *ctx, int32_t *out0, const
                          struct futhark_i32_1d *in0, const
                          struct futhark_i32_1d *in1);
int futhark_entry_test_process(struct futhark_context *ctx, int32_t *out0, const
                               struct futhark_i32_1d *in0, const
                               struct futhark_i32_1d *in1);
...
static void futrts_cli_entry_process(struct futhark_context *ctx)
{
    ...
    struct futhark_i32_1d *read_value_4411;
    int64_t read_shape_4412[1];
    int32_t *read_arr_4413 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4413, read_shape_4412, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 0,
                      "[]", i32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *read_value_4414;
    int64_t read_shape_4415[1];
    int32_t *read_arr_4416 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4416, read_shape_4415, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 1,
                      "[]", i32_info.type_name, strerror(errno));
    if (end_of_input() != 0)
    ...
    if (perform_warmup) {
        ...
        assert((read_value_4411 = futhark_new_i32_1d(ctx, read_arr_4413,
                                                     read_shape_4412[0])) != 0);
        assert((read_value_4414 = futhark_new_i32_1d(ctx, read_arr_4416,
                                                     read_shape_4415[0])) != 0);
        ...
        r = futhark_entry_process(ctx, &result_4417, read_value_4411,
                                  read_value_4414);
        if (r != 0)
            futhark_panic(1, "%s", futhark_context_get_error(ctx));
        if (futhark_context_sync(ctx) != 0)
        ...
        assert(futhark_free_i32_1d(ctx, read_value_4411) == 0);
        assert(futhark_free_i32_1d(ctx, read_value_4414) == 0);
        ;
    }
    time_runs = 1;
    // Proper run.
    ...
    write_scalar(stdout, binary_output, &i32_info, &result_4417);
    printf("\n");
    ;
}
static void futrts_cli_entry_test_process(struct futhark_context *ctx)
{
    int64_t t_start, t_end;
    int time_runs = 0, profile_run = 0;
    ...
    struct futhark_i32_1d *read_value_4418;
    int64_t read_shape_4419[1];
    int32_t *read_arr_4420 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4420, read_shape_4419, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 0,
                      "[]", i32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *read_value_4421;
    int64_t read_shape_4422[1];
    int32_t *read_arr_4423 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4423, read_shape_4422, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 1,
                      "[]", i32_info.type_name, strerror(errno));
    if (end_of_input() != 0)
    ...
    if (perform_warmup) {
        ...
        assert((read_value_4418 = futhark_new_i32_1d(ctx, read_arr_4420,
                                                     read_shape_4419[0])) != 0);
        assert((read_value_4421 = futhark_new_i32_1d(ctx, read_arr_4423,
                                                     read_shape_4422[0])) != 0);
        if (futhark_context_sync(ctx) != 0)
            futhark_panic(1, "%s", futhark_context_get_error(ctx));
        ;
        ...
        if (profile_run)
            futhark_context_unpause_profiling(ctx);
        t_start = get_wall_time();
        r = futhark_entry_test_process(ctx, &result_4424, read_value_4418,
                                       read_value_4421);
        if (r != 0)
            futhark_panic(1, "%s", futhark_context_get_error(ctx));
        if (futhark_context_sync(ctx) != 0)
        ...
        assert(futhark_free_i32_1d(ctx, read_value_4418) == 0);
        assert(futhark_free_i32_1d(ctx, read_value_4421) == 0);
        ;
    }
    time_runs = 1;
    // Proper run.
    ...
    if (binary_output)
        set_binary_mode(stdout);
    write_scalar(stdout, binary_output, &i32_info, &result_4424);
    printf("\n");
    ;
}
...
int main(int argc, char **argv)
{
    ...
    struct entry_point_entry entry_points[] = {{.name ="process", .fun =
                                                futrts_cli_entry_process},
                                               {.name ="test_process", .fun =
                                                futrts_cli_entry_test_process}};
...
static int futrts_process(struct futhark_context *ctx,
                          int32_t *out_scalar_out_4409,
                          struct memblock signal1_mem_4405,
                          struct memblock signal2_mem_4406, int64_t n_4373,
                          int64_t n_4374);
static int futrts_test_process(struct futhark_context *ctx,
                               int32_t *out_scalar_out_4410,
                               struct memblock signal1_mem_4405,
                               struct memblock signal2_mem_4406, int64_t n_4388,
                               int64_t n_4389);
                          ...
                          int64_t n_4374)
{
    ...
    int err = 0;
    int32_t scalar_out_4407;
    bool dim_match_4377 = n_4373 == n_4374;
    bool empty_or_match_cert_4378;
    
    if (!dim_match_4377) {
        ctx->error = msgprintf("Error: %s\n\nBacktrace:\n%s",
                               "function arguments of wrong shape",
                               "-> #0  exercise1.fut:19:1-20:59\n");
        return 1;
    }
    
    int32_t res_4380;
    int32_t redout_4403 = 0;
    
    for (int64_t i_4404 = 0; i_4404 < n_4373; i_4404++) {
        int32_t x_4384 = ((int32_t *) signal1_mem_4405.mem)[i_4404];
        int32_t x_4385 = ((int32_t *) signal2_mem_4406.mem)[i_4404];
        int32_t res_4386 = sub32(x_4384, x_4385);
        int32_t res_4387 = abs(res_4386);
        int32_t res_4383 = smax32(res_4387, redout_4403);
        int32_t redout_tmp_4408 = res_4383;
        
        redout_4403 = redout_tmp_4408;
    }
    res_4380 = redout_4403;
    scalar_out_4407 = res_4380;
    *out_scalar_out_4409 = scalar_out_4407;
                               ...
                               int64_t n_4389)
{
    ...
    int err = 0;
    int32_t scalar_out_4407;
    bool dim_match_4392 = n_4388 == n_4389;
    bool empty_or_match_cert_4393;
    
    if (!dim_match_4392) {
        ctx->error = msgprintf("Error: %s\n\nBacktrace:\n%s",
                               "function arguments of wrong shape",
                               "-> #0  exercise1.fut:22:1-23:25\n");
        return 1;
    }
    ...
    for (int64_t i_4404 = 0; i_4404 < n_4388; i_4404++) {
        int32_t x_4399 = ((int32_t *) signal1_mem_4405.mem)[i_4404];
        int32_t x_4400 = ((int32_t *) signal2_mem_4406.mem)[i_4404];
        int32_t res_4401 = sub32(x_4399, x_4400);
        int32_t res_4402 = abs(res_4401);
        int32_t res_4398 = smax32(res_4402, redout_4403);
        int32_t redout_tmp_4408 = res_4398;
        
        redout_4403 = redout_tmp_4408;
    }
    res_4395 = redout_4403;
    scalar_out_4407 = res_4395;
    *out_scalar_out_4410 = scalar_out_4407;
                          ...
                          struct futhark_i32_1d *in1)
{
    ...
    signal1_mem_4405.references = NULL;
    
    struct memblock signal2_mem_4406;
    
    signal2_mem_4406.references = NULL;
    ...
    lock_lock(&ctx->lock);
    signal1_mem_4405 = in0->mem;
    n_4373 = in0->shape[0];
    signal2_mem_4406 = in1->mem;
    n_4374 = in1->shape[0];
    
    int ret = futrts_process(ctx, &scalar_out_4407, signal1_mem_4405,
                             signal2_mem_4406, n_4373, n_4374);
    
    if (ret == 0) {
        *out0 = scalar_out_4407;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
                               ...
                               struct futhark_i32_1d *in1)
{
    ...
    int ret = futrts_test_process(ctx, &scalar_out_4407, signal1_mem_4405,
                                  signal2_mem_4406, n_4388, n_4389);
